    ...
    <body>
                                ...
                                <div class="span12">
                                    <div class="box dark">
                                        ...
                                        <div  id="div-1"  class="accordion-body collapse in body">
                                            <table id="templateDataTable" class="table table-bordered table-condensed table-hover table-striped">
                                                <thead>
                                                    <tr>
                                                    	<th>序号</th>
                                                    	<th>模板名称</th>
                                                ...
                                                <tbody>
                                                <s:iterator value="%{resultMap.resultList}" id="template"> 
												    <tr dataid=<s:property value="#template.templateId" />>
												    <td><s:property value="#template.templateId" /></td>
												    	<td><s:property value="#template.templateName" /></td>
                                                    	<td><s:property value="#template.typeDesc" /></td>
                                                        <td><s:property value="#template.path" /></td>
                                                        <td><s:property value="#template.open" /></td>
                                                        <td><s:property value="#template.templateFileName" /></td>
                                                        <td><s:date name="#template.ctime"  format="yy-MM-dd" /></td>
                                                        <td><s:date name="#template.mtime"  format="yy-MM-dd HH:mm" /></td>
                                                        <td>
                                                        	<button class="btn edit"  dataid=<s:property value="#template.templateId" />><i class="icon-edit" ></i></button>
                                                            <button class="btn btn-danger remove" dataid=<s:property value="#template.templateId" />><i class="icon-remove" ></i></button>
                                                        </td>
                                                    </tr>
												</s:iterator>
                                                </tbody>
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('#templateDataTable tbody').on( 'click', 'button.view', function () {
                	$.ajax({
                		type:'post',
                		url:'api/template_get.do',
                ...
                $('#templateDataTable tbody').on( 'click', 'button.edit', function () {
                    location.href = "template_modify.do?template.templateId="+$(this).attr("dataid");
                }); 
                $('#templateDataTable tbody').on( 'click', 'button.remove', function () {
                    var idsvalue = $(this).attr("dataid");
        			bootbox.confirm("您确定要删除操作吗?", function(result) {
            			if(result){
                        	$.ajax({
                        		type:'post',
                        		url:'api/template_delete.do',
                        		data:{ids:idsvalue},
                        		dataType:'json',
                        		beforeSend: function(){
                        		},
