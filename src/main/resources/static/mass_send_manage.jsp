        ...
        <script type="text/javascript">
            ...
            $(function() {
            	...
            	$('#textDataTable').dataTable({
                	"processing": true,
                    "serverSide": true,
                    "ajax": "api/text_list_mass_send.do",
                    "columns": [
                        { "data": "id" },
                        { "data": "messageShow"},
                        { "data": "ctime" },
                    ...
                    "columnDefs": [  
                        ...
                        { "targets": [6],
                        "data": "id" ,
                        "render": function(data, type, full) { return "<!--<button class='btn edit'  dataid='"+data+"'><i class='icon-edit' ></i></button>--> <button class='btn send'  dataid='"+data+"'><i class='icon-envelope' title='群发消息'></i></button>"; } 
                        } 
                    ],
                ...
                $('#textDataTable tbody').on( 'click', 'button.send', function () {
                    var msgId = $(this).attr("dataid");
                    var groupId = $("#group_"+msgId).val();
                    //alert("msgId : " + msgId);
                    //alert("groupId : " +groupId);
        			bootbox.confirm("您确定要将此条消息群发到选定的用户组吗?", function(result) {
            			if(result){
                        	$.ajax({
                        		type:'post',
                        		url:'api/mass_send.do',
                        		data:{msgType:"text",id:msgId,"groupId":groupId},
                        		dataType:'json',
                        		beforeSend: function(){
                        		},
            	...
            	$('#newsDataTable').dataTable({
                	"processing": true,
                    "serverSide": true,
                    "ajax": "api/news_list_mass_send.do",
                    "columns": [
                        { "data": "id" },
                        { "data": "messageShow"},
                        { "data": "ctime" },
                    ...
                    "columnDefs": [  
                        ...
                        { "targets": [6],
                        "data": "id" ,
                        "render": function(data, type, full) { return "<button class='btn send'  dataid='"+data+"'><i class='icon-envelope' title='群发消息'></i></button>"; } 
                        } 
                    ],
                ...
                $('#newsDataTable tbody').on( 'click', 'button.send', function () {
                    var msgId = $(this).attr("dataid");
                    var groupId = $("#group_"+msgId).val();
        			bootbox.confirm("您确定要将此条消息群发到选定的用户组吗?", function(result) {
            			if(result){
                        	$.ajax({
                        		type:'post',
                        		url:'api/mass_send.do',
                        		data:{msgType:"news", id:msgId, "groupId":groupId},
                        		dataType:'json',
                        		beforeSend: function(){
                        		},
                        		success:function(data){
