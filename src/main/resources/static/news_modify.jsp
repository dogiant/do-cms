...
<!--[if gt IE 8]><!-->  <html class="no-js">                        <!--<![endif]-->
    ...
    <body>
                                    ...
                                    <div class="box dark">
                                        ...
                                        <div id="div-1" class="accordion-body collapse in body">
                                        	<div class="span12">
	                                        	...
	                                        	<div class="span4">							                
												<s:if test="%{resultMap.news.newsAticles.size()>0}">
									                <div class="media_preview_area">
												        <div class="appmsg multi editing">
												            <div id="js_appmsg_preview" class="appmsg_content">
												            
												            <s:iterator id="obj" value="resultMap.news.newsAticles" status="st">
												            	<s:if test="%{#st.index+1==1}">
												                <div id="appmsgItem<s:property value="#st.index+1"/>" data-fileid="" data-id="<s:property value="#st.index+1"/>" class="js_appmsg_item ">
															        <div class="appmsg_info">
															            <em class="appmsg_date"></em>
															        </div>
															        <div class="cover_appmsg_item">
															            <h4 class="appmsg_title"><a href="javascript:void(0);" onclick="return false;" target="_blank"><s:property value="#obj.articleItem.title"/></a></h4>
															            <div class="appmsg_thumb_wrp">
															                <img class="js_appmsg_thumb appmsg_thumb" src="http://file.dogiant.com<s:property value="#obj.articleItem.topCover"/>">
															                <i class="appmsg_thumb default">封面图片</i>
															            </div>
															            <div class="appmsg_edit_mask">
															                <a onclick="return false;" class="icon18_common edit_gray js_del" data-id="<s:property value="#st.index+1"/>" href="javascript:;">删除</a>
															            </div>
															        </div>
																</div>
																</s:if>
																<s:else>
																<div id="appmsgItem<s:property value="#st.index+1"/>" data-fileid="" data-id="<s:property value="#st.index+1"/>" class="appmsg_item js_appmsg_item ">
																    <img class="js_appmsg_thumb appmsg_thumb" src="http://file.dogiant.com<s:property value="#obj.articleItem.normalThumbnail"/>">
																    <i class="appmsg_thumb default">缩略图</i>
																    <h4 class="appmsg_title"><a onclick="return false;" href="javascript:void(0);" target="_blank"><s:property value="#obj.articleItem.adaptiveTitle"/></a></h4>
																    <div class="appmsg_edit_mask">
																        <a class="icon18_common del_gray js_del" data-id="<s:property value="#st.index+1"/>" onclick="return false;" href="javascript:void(0);">删除</a>
																    </div>
																</div>
																</s:else>
																</s:iterator>
																<s:set id="num" value="resultMap.news.newsAticles.size()+1"></s:set>	
																<s:iterator id="i" begin="num" end="10" status="st">
																<div id="appmsgItem<s:property value="#i"/>" data-fileid="" data-id="<s:property value="#i"/>" class="appmsg_item js_appmsg_item " style="display:none;">
																    ...
																    <div class="appmsg_edit_mask">
																        ...
																        <a class="icon18_common del_gray js_del" data-id="<s:property value="#i"/>" onclick="return false;" href="javascript:void(0);">删除</a>
																    </div>
																</div>
																</s:iterator>																																														
															</div>
													          <div class="appmsg_add icon24_common">
													          	<form id="multiNewsForm">
													          		<input type="hidden" name="news.id" value="<s:property value="resultMap.news.id"/>" />
													                <a onclick="return false;" id="js_add_appmsg" href="javascript:void(0);">
													                		<span id="news_ids">
													                		<s:iterator id="obj" value="resultMap.news.newsAticles" status="st"> 
													                			<input type="hidden" name="articleItemIds" value="<s:property value="#obj.articleItem.id"/>">
													                		</s:iterator>
													                		</span>
													                    	<button class="btn btn-large" type="submit"  id="multiNewsUpdate">完成修改组合提交</button>
													                </a>
													           	</form>
													           </div>
												           </div>
									                ...
									                <s:else>
											            <div class="thumbnail" id="news_thumbnail">
			                                        	  ...
			                                        	  <div id="cover_wrapper">
										                  	<img src="http://file.dogiant.com<s:property value="%{resultMap.articleItem.picUrl}"/>" id="news_cover">
										                  	<i>封面图片</i>                           	  
			                                        	  </div>
										                  <div class="caption">
										                    <p id="news_digest" >
		...
		$().ready(function() {
	        $("#news_ids input").each(function(index){
	        	var id = Number(index + 1);
		    	$("#appmsgItem"+id).find(".default").hide();
		    	$("#appmsgItem"+id).find(".js_appmsg_thumb").show();
		 	});
            ...
            $('#multiNewsUpdate').click(function () {
            	//var data = JSON.stringify($("#multiNewsForm").serializeArray());
            	var data = $("#multiNewsForm").serialize();
            	
				//判断提交数据的长度
				if(data.length<1){
					alert("您需要选择至少一个图文项，请点击操作按钮增加图文项至图文组合!");
					return false;
				}
    			bootbox.confirm("您确定要提交修改此图文消息吗?", function(result) {
        			if(result){
                    	$.ajax({
                    		type:'post',
                    		url:'api/news_update.do',
                    		data:data,
                    		dataType:'json',
                    		beforeSend: function(){
                    		},
                    		success:function(data){
                    			if(data.success){
                        			//alert("成功修改图文消息");
                					bootbox.alert('您的图文消息已成功修改' ,function(){
