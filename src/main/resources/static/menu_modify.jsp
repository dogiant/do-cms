...
<!--[if gt IE 8]><!-->  <html class="no-js">                        <!--<![endif]-->
    ...
    <body>
                                ...
                                <div class="span12">
                                    <div class="box dark">
                                        <header>
                                            ...
                                            <div class="toolbar">
                                                <ul class="nav">
                                                    <li><a href="menu_list.do">自定义菜单列表</a></li>
                                                    <li class="dropdown">
                                        ...
                                        <div id="div-1" class="accordion-body collapse in body">
                                        	...
                                        	<div class="form-horizontal">
                                                <s:form id="menuForm" namespace="/api" action="menu_update" method="post" > 
                                                <s:hidden  id="menuId" name="customMenu.id" value="%{resultMap.customMenu.id}"></s:hidden>
                                                <div class="control-group">
                                                    <label for="name" class="control-label">上级菜单</label>
                                                    <div class="controls">
                                                    	<s:select name="customMenu.parent.id" headerValue="顶级菜单" headerKey="%{null}" list="customMenuList" listKey="id" listValue="name" emptyOption="false" value="%{resultMap.customMenu.parent.id}"></s:select>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="control-group">
                                                    ...
                                                    <div class="controls">
                                                    	<s:textfield id="name" name="customMenu.name" class="span6 input-tooltip"
                                                               data-original-title="请输入菜单名称" data-placement="bottom" value="%{resultMap.customMenu.name}"></s:textfield>
                                                    </div>
                                                </div>
                                                <div class="control-group">
														<label class="control-label">菜单类型</label>
														<div class="controls controls-row">
																	<input class="uniform" type="radio" name="customMenu.type" value="click"  <s:if test='%{resultMap.customMenu.type=="click"}'>checked="checked"</s:if>>关键词
                                                                    <input class="uniform" type="radio" name="customMenu.type" value="view"  <s:if test='%{resultMap.customMenu.type=="view"}'>checked="checked"</s:if>>网页链接        
													</div>
                                               </div>
                                                <div class="control-group" id="keyControl">
                                                    <label for="key" class="control-label">关键词</label>
                                                    <div class="controls">
                                                    	<s:textfield id="key" name="customMenu.key" class="span6 input-tooltip"
                                                               data-original-title="请输入关键词" data-placement="bottom"  value="%{resultMap.customMenu.key}"></s:textfield>
                                                    </div>
                                                </div>
                                                <div class="control-group" id="urlControl" style="display: none;">
                                                    <label for="url" class="control-label">网页链接</label>
                                                    <div class="controls">
                                                    	<s:textfield id="url" name="customMenu.url" class="span6 input-tooltip"
                                                               data-original-title="请输入网页链接" data-placement="bottom" value="%{resultMap.customMenu.url}"></s:textfield>
                                                    </div>
                                                </div>
                                                <div class="control-group">
                                                    <label for="sortOrder" class="control-label">排序字段</label>
                                                    <div class="controls">
                                                    	<s:textfield id="sortOrder" name="customMenu.sortOrder" class="span6 input-tooltip"
                                                               data-original-title="请输入排序字段" data-placement="bottom" value="%{resultMap.customMenu.sortOrder}"></s:textfield>
                                                    </div>
                                                </div>
        ...
        $(function(){
        	 showCont();
        	 $("input[name='customMenu.type']").click(function(){
        	 	showCont();
        	 });
        });
        function showCont(){
        	 switch($("input[name='customMenu.type']:checked").attr("value")){
        	  case "click":
		       	   $("#urlControl").hide();
		       	   $("#keyControl").show();
		       	   break;
		...
		$().ready(function() {
			 $("#menuForm").validate({
					...
					messages: {
						...
						"customMenu.sortOrder":  {
							required: "请输入排序字段数字，菜单根据排序字段正序排列。",
							number:"排序字段必须是数字，菜单项根据数字正序排列。"
						}
					},
			 	...
			 	function showResponse(data)  { 
				 	if(data.success){
						bootbox.alert('成功修改自定义菜单' ,function(){
							message_box.show('将跳转到您的自定义菜单列表管理界面!','success');
